...
class UnifiedScanQADataset(ScanNetBaseDataset):
    """
    Unified ScanQA dataset that supports both standard training and RL training
    ...
    def __getitem__(self, idx):
        ...
        if not self.use_rl_training:
            ...
            ret_dict['target_text'] = answer  # Target text for reward computation
            ret_dict['target_response'] = response  # Full target response
            ret_dict['question'] = question  # Question for context
            ret_dict['object_ids'] = target_obj_id  # Object IDs for reference
...
class Dataset(UnifiedScanQADataset):
